---
layout: default
title: Research
body_class: index-page
---

<header class="page-mast">
  <div class="mast-title"><h1>Research</h1></div>
  <div class="mast-copy"><p class="page-deck">Published studies and experimental notes connect Windows Phone sources to the Flutter implementation. Their status stays explicit.</p></div>
</header>

<section class="manual-section" aria-labelledby="published-title">
  <header class="section-label"><h2 id="published-title">Published</h2></header>
  <div class="section-body">
    <a class="featured-record" href="{{ '/research/pivot/' | relative_url }}">
      <span class="record-name">Pivot motion</span>
      <span class="record-detail">57 WP8.1 emulator trials, Flutter comparisons, raw evidence, and recorded limits.</span>
      <span class="status-badge status-available">Published</span>
    </a>
    <a class="featured-record" href="{{ '/research/components/micro-controls/' | relative_url }}">
      <span class="record-name">Micro controls</span>
      <span class="record-detail">Slider, ProgressBar, and TiltEffect across 17 WP8.1 emulator trials and retained Flutter input replays.</span>
      <span class="status-badge status-available">Published</span>
    </a>
  </div>
</section>

<section class="manual-section" aria-labelledby="working-title">
  <header class="section-label"><h2 id="working-title">In progress</h2></header>
  <div class="section-body">
    <a class="featured-record" href="{{ '/research/components/' | relative_url }}">
      <span class="record-name">Experimental components</span>
      <span class="record-detail">Application Bar, Panorama, and Toggle Switch pilots plus links to the calibrated micro-control batch.</span>
      <span class="status-badge">Research notes</span>
    </a>
    <p class="quiet-note">The earlier batch still needs calibrated motion. Lumia validation remains open for every study.</p>
  </div>
</section>

<section class="manual-section" aria-labelledby="rule-title">
  <header class="section-label"><h2 id="rule-title">Publication rule</h2></header>
  <div class="section-body"><p>A finding needs a labeled source, capture reference, method, and raw data. Until then, it is planned or in progress.</p></div>
</section>
